  ...
  run: function(creep) {
    ...
    try {
      ...
      if (creep.memory.harvest) {
        const target = actionLookup.findDroppedSources(creep.room);
        if (target.length > 0) {
          if (creep.pickup(target[0]) === ERR_NOT_IN_RANGE) {
            actionMove.run(creep, Game.getObjectById(target[0]), '#ffaa00');
          }
        } else {
          var sources = actionLookup.findSources(creep.room);
        ...
        var targets = actionLookup.findHarvesterDumpSites(creep.room);
